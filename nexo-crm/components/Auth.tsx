...
import { Mail, Lock, User, ArrowRight, ShieldCheck, ChevronLeft, XCircle, AlertCircle, CheckCircle2, X, Sun, Moon } from 'lucide-react';
...
const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen bg-zinc-100 dark:bg-[#09090b] flex items-center justify-center p-6 relative overflow-hidden transition-colors duration-300">
      ...
      <button
        onClick={toggleTheme}
        className="absolute top-6 right-6 p-3 text-zinc-500 dark:text-zinc-400 hover:text-indigo-500 dark:hover:text-indigo-400 transition-colors bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl backdrop-blur-sm z-20 shadow-sm"
      ...
      <div className="w-full max-w-md z-10">
        <div className="flex justify-center mb-8">
          <div className="bg-transparent">
            <img src="/logo-nero.png" alt="Nero Logo" className="w-12 h-12" />
          </div>
        </div>

        <div className="bg-white/80 dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800/50 p-8 rounded-[32px] backdrop-blur-xl shadow-2xl transition-colors duration-300">
          <div className="text-center mb-8">
            <h1 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white mb-2">{title.main}</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === 'register' && (
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <User className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400 dark:text-zinc-600" size={18} />
                  <input
                    required
                    type="text"
                    value={name}
                    ...
                    className="w-full bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl pl-10 pr-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500/50 transition-all text-zinc-900 dark:text-white placeholder:text-zinc-400 dark:placeholder:text-zinc-700"
                  />
                </div>
              </div>
            )}
            ...
            {mode === 'register' && (
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <ShieldCheck className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400 dark:text-zinc-600" size={18} />
                  <input
                    required
                    type="text"
                    value={companyName}
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400 dark:text-zinc-600" size={18} />
                <input
                  required
                  type="email"
                  value={email}
            ...
            {mode !== 'recover' && (
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400 dark:text-zinc-600" size={18} />
                  <input
                    required
                    type="password"
                    value={password}
          ...
          <div className="mt-8 pt-6 border-t border-zinc-200 dark:border-zinc-800/50 text-center">
            {mode === 'login' ? (
              <p className="text-sm text-zinc-500">
                Não tem uma conta?{' '}
                <button onClick={() => setMode('register')} className="text-indigo-500 dark:text-indigo-400 hover:text-indigo-600 dark:hover:text-indigo-300 font-medium">Cadastre-se grátis</button>
              </p>
            ) : (
              <button
                onClick={() => setMode('login')}
                className="text-sm text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white flex items-center justify-center gap-2 mx-auto transition-colors"
              >
        ...
        <p className="text-center text-[11px] text-zinc-500 dark:text-zinc-600 mt-8 uppercase tracking-widest font-semibold">
      ...
      {modal.isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 dark:bg-black/70 backdrop-blur-sm transition-all animate-in fade-in duration-200">
          <div className="w-full max-w-sm bg-white dark:bg-[#0c0c0e] border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] p-8 shadow-[0_0_50px_rgba(0,0,0,0.2)] dark:shadow-[0_0_50px_rgba(0,0,0,0.5)] animate-in zoom-in-95 duration-200 text-center">
            ...
            <h3 className="text-xl font-bold text-zinc-900 dark:text-white mb-3">{modal.title}</h3>
