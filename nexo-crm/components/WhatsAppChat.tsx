...
const WhatsAppChat: React.FC<WhatsAppChatProps> = ({ leads, onLeadsUpdate, selectedChatId, onSelectChat, chatTableName: propChatTableName }) => {
  ...
  const [assignmentsMap, setAssignmentsMap] = useState<Record<string, TenantUser | null>>({});
  ...
  useEffect(() => {
    if (tenantId || effectiveUserId) {
      const cacheKey = `nero_tenant_members_${tenantId || effectiveUserId}`;
      // Restaurar do cache primeiro
      const cached = localStorage.getItem(cacheKey);
      if (cached) {
        try {
          setTenantMembers(JSON.parse(cached));
        } catch (e) { }
      }

      // Buscar do banco e atualizar cache
      tenantService.listAtendentes(tenantId || effectiveUserId).then(data => {
  ...
  useEffect(() => {
    const loadAllAssignments = async () => {
      const map: Record<string, TenantUser | null> = {};
      for (const lead of leads) {
        if (lead.assigned_to) {
          const member = tenantMembers.find(m => m.id === lead.assigned_to) || null;
          map[lead.id] = member;
        }
      }
  ...
  const handleSendMessage = async () => {
    ...
    if (userType === 'atendente' && userInfo && !currentAssignment) {
      await tenantService.assignLeadToMember(selectedChat.id, userInfo.tenantUserId);
      // Criar objeto TenantUser para atualização local
      const selfAsMember: TenantUser = {
        id: userInfo.tenantUserId,
      ...
      setCurrentAssignment(selfAsMember);
    }
  ...
  const handleAssignLead = async (memberId: string | null) => {
    ...
    await tenantService.assignLeadToMember(selectedChat.id, memberId);

    if (memberId) {
      const assigned = tenantMembers.find(m => m.id === memberId);
  ...
  if (!initialLeadsLoaded && leads.length === 0) {
    return (
      <div className="h-full flex items-center justify-center bg-white dark:bg-[#0b141a]">
        <div className="text-center p-8">
          <div className="w-12 h-12 border-4 border-emerald-500 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
      ...
      <div className="w-80 border-r border-zinc-800/50 flex flex-col bg-white dark:bg-[#0b141a]">
        <div className="p-4 border-b border-zinc-800/50">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-zinc-100 dark:bg-[#182229] border-none rounded-xl text-xs text-zinc-800 dark:text-zinc-100 focus:outline-none focus:ring-1 focus:ring-indigo-500/50 transition-all"
            />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto">
          {leads.filter(chat => {
            ...
            // Ordenar por last_message_at (última mensagem) - mais recente primeiro
            ...
            <button
              ...
              className={`w-full p-4 flex gap-3 items-center hover:bg-zinc-50 dark:hover:bg-white/5 transition-all border-b border-zinc-800/10 text-left relative overflow-hidden
                  ${selectedChatId === chat.id ? 'bg-indigo-50 dark:bg-white/10 border-l-4 border-l-indigo-500 shadow-inner' : ''}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-baseline mb-1">
                  <div className="flex items-center gap-2 min-w-0">
                    <h4 className="text-[13px] font-semibold truncate text-zinc-900 dark:text-zinc-200">{getLeadDisplayName(chat)}</h4>
                  ...
                  <span className="text-[10px] text-zinc-500 shrink-0">
                    {chat.last_message_at || chat.created_at
                      ? new Date(chat.last_message_at || chat.created_at!).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })
                      : 'Agora'}
                  </span>
                </div>
                ...
                {assignmentsMap[chat.id] && (
                  <div className="flex items-center gap-1 mt-1">
                    ...
                    <span className="text-[9px] text-indigo-400 font-medium">
                      {assignmentsMap[chat.id]?.id === userInfo?.tenantUserId
                        ? 'Atribuído a você'
        ...
        <div className="flex-1 flex flex-col bg-white dark:bg-[#0b141a]">
          <header className="h-[64px] px-4 border-b border-zinc-800/30 flex items-center justify-between bg-white dark:bg-[#0b141a] z-10">
            ...
            <div className="flex items-center gap-3 text-zinc-400 px-2">
              ...
              {userType === 'admin' && tenantMembers.length > 0 && (
                <div className="relative">
                  <button
                  ...
                  {showAssignDropdown && (
                    <div className="absolute top-full right-0 mt-2 w-48 bg-[#0d1419] border border-zinc-700/50 rounded-xl shadow-xl z-50 overflow-hidden">
                      ...
                      {tenantMembers.filter(m => m.ativo).map(member => (
                        <button
                          key={member.id}
              ...
              {userType === 'atendente' && currentAssignment && (
                <div className="flex items-center gap-2 px-3 py-1.5 bg-indigo-500/10 text-indigo-400 rounded-full text-xs font-semibold border border-indigo-500/20">
                  <UserPlus size={12} />
                  <span>{currentAssignment.id === userInfo?.tenantUserId ? 'Você' : currentAssignment.nome.split(' ')[0]}</span>
                </div>
              )}
          ...
          <div
            ...
            className="flex-1 overflow-y-auto p-4 md:p-8 space-y-4 relative bg-white dark:bg-[#0b141a]"
          >
                ...
                <>
                  ...
                  {selectedChat?.notifica_humano && (
                    <div className="sticky bottom-4 left-0 right-0 z-50 px-4 pb-4 flex justify-center animate-in slide-in-from-bottom-4 duration-500">
                      <div className="bg-white dark:bg-[#2a3942] border border-zinc-200 dark:border-rose-500/30 rounded-2xl shadow-2xl p-5 max-w-lg w-full flex flex-col gap-4 ring-1 ring-zinc-200 dark:ring-rose-500/20 backdrop-blur-md bg-opacity-95 dark:bg-opacity-95">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <div className="p-2 bg-rose-500/10 dark:bg-rose-500/20 rounded-lg">
                              <Bot size={18} className="text-rose-500 dark:text-rose-400" />
                            </div>
                            <div>
                              <h5 className="text-[13px] font-bold text-zinc-800 dark:text-zinc-100">Intervenção Humana Solicitada</h5>
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          <button
                            onClick={handleToggleAI}
                            className="flex-1 py-2.5 px-4 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-200 text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-2 border border-zinc-200 dark:border-transparent"
                          >
                          ...
                          <button
                            ...
                            className="flex-[2] py-2.5 px-4 bg-rose-500 hover:bg-rose-600 text-white text-xs font-bold rounded-xl transition-all shadow-lg shadow-rose-900/10 dark:shadow-rose-900/20 flex items-center justify-center gap-2"
                          >
                            <CheckCheck size={16} />
          ...
          <footer className="p-3 bg-white dark:bg-[#0b141a] border-t border-zinc-800/30 z-10">
            {!canSendMessage ? (
              ...
              >
                ...
                <div className="flex-1 relative">
                  <input
                    ...
                    className="w-full py-2 px-4 bg-zinc-100 dark:bg-[#2a3942] border-none rounded-xl text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-0"
                  />
                </div>
                <button
                  type="submit"
